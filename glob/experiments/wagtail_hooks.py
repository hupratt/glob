...
from glob.experiments import admin_urls
...
class ExperimentButtonHelper(ButtonHelper):
    ...
    def get_buttons_for_obj(
        ...
        btns = super(ExperimentButtonHelper, self).get_buttons_for_obj(
            obj, exclude, classnames_add, classnames_exclude
        )

        if "report" not in exclude and ph.user_can_edit_obj(self.request.user, obj):
            btns.append(self.report_button(pk, classnames_add, classnames_exclude))
        return btns
...
def check_experiments(page, request, serve_args, serve_kwargs):
    ...
    experiments = Experiment.objects.filter(
        control_page=page, status__in=("live", "completed")
    )
    if experiments:
        ...
        if (
            ...
            and experiment.winning_variation is not None
        ):
