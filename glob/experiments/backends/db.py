...
def record_participant(experiment, user_id, variation, request):
    from glob.experiments.models import ExperimentHistory
...
def get_report(experiment):
    ...
    for variation in variations:
        history_entries = ExperimentHistory.objects.filter(
        ...
        variation_data = {
            ...
            "is_winner": variation.pk == experiment.winning_variation.pk
            if experiment.winning_variation
            else False,
            "total_participant_count": history_entries.aggregate(
                sum=Sum("participant_count")
            ).get("sum", 0),
            "total_completion_count": history_entries.aggregate(
                sum=Sum("completion_count")
            ).get("sum", 0),
        }
